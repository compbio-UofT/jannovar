...
public class UCSCKGParser extends TranscriptDataParser implements Constants {
	...
	/**
	 * This function causes all four UCSC files to be parsed. This function is used for uncompressed file (i.e., without
	 * the .gz suffix). This results in the construction of {@link #knownGeneMap}.
	 */
	 ...
	 */
	public TranscriptModel parseTranscriptModelFromLine(String line) throws JannovarException {
		TranscriptModel model = TranscriptModel.createTranscriptModel();
		String A[] = line.split("\t");
		if (A.length != NFIELDS) {
			String error = String.format("Malformed line in UCSC knownGene.txt file:\n%s\nExpected %d fields but there were %d", line, NFIELDS, A.length);
		...
		if (B.length != exonCount) {
			String error = String.format("[UCSCKGParser] Malformed exonStarts list: found %d but I expected %d exons", B.length, exonCount);
	 ...
	 * @param isGzip
	 *            <code>true</code> if gzipped
	 * @throws jannovar.exception.KGParseException
	 */
	public void parseKnownGeneFile(String kgpath, boolean isGzip) throws JannovarException {
		// int linecount=0;
	 ...
	 */
	private void parseKnown2Locus(String locuspath, boolean isGzip) throws JannovarException {
		try {
	 ...
	 * TranscriptModel} objects.
	 */
	private void parseKnownGeneMrna(String mrnapath, boolean isGzip) throws JannovarException {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (A.length != 2) {
					...
					s += ("[ERROR] Got " + A.length + " fields instead of the expected 2");
					s += ("[ERROR] Fix problem in UCSC file before continueing");
                    LOG.error(s);
                    throw new JannovarException(s);
				}
			...
			br.close();
			LOG.error(String.format("[INFO] Found %d transcript models from UCSC KnownGenes resource, %d of which had sequences", foundSequence, (foundSequence - kgWithNoSequence)));
		} catch (FileNotFoundException fnfe) {
